...
public class AccessToken {
    ...
    private void signatureValidation() throws java.text.ParseException, URISyntaxException, IOException, InterruptedException, JOSEException {
        try {
            ...
            validationErrors += "Signature is not valid\n";
        }
    }
    ...
    public void validateToken() throws URISyntaxException, IOException, InterruptedException, AuthenticationException, java.text.ParseException, JOSEException {
        ...
        if(!(jsonToken.get("iss").equals(discovery.getIssuer()))){
            validationErrors += "issuer is not the OpenID Connect Provider\n";
            //throw new AuthenticationException("issuer is not the OpenID Connect Provider");
        }
        ...
        if(aud instanceof String){
            if(!(aud.equals(clientId))){
                validationErrors += "ClientId is not the same as own ClientID\n";
                //throw new AuthenticationException("ClientId is not the same as own ClientID");
            }
        }
        else {
            if (((JSONArray) aud).contains(clientId)){
                validationErrors += "ClientID is not the same as own ClientID\n";
                //throw new AuthenticationException("ClientId is not the same as own ClientID");
            }
        }
        ...
        //expiry time
        if((long) jsonToken.get("exp") < (System.currentTimeMillis()/1000)){
            validationErrors += "Expiry time is greater than current timestamp\n";
            //throw new AuthenticationException("Expiry time is greater than current timestamp");
        }

        //issued at not too long ago
        if((long) jsonToken.get("iat") + issuedAtTime < (System.currentTimeMillis()/1000)){
            validationErrors += "Issued time is too long ago\n";
            //throw new AuthenticationException("Issued time is too long ago");
        }
    ...
    public void validateToken(String nonce) throws AuthenticationException, URISyntaxException, IOException, ParseException, InterruptedException, JOSEException {
        ...
        //nonce
        if(!jsonToken.get("nonce").equals(nonce)){
            //throw new AuthenticationException("value of nonce is not the same as in Authentication Request");
            validationErrors += "value of nonce is not the same as in Authentication Request\nvalue of nonce in Authentication: "+ nonce+", value of nonce in token: "+ jsonToken.get("nonce")+"\n";
        }
